...
서버사이드 프로그래밍을 사용하면 정보를 데이터베이스에 대신 저장하고 HTML 또는 기타 유형의 파일 (예: PDF, 이미지 등)을 동적으로 생성하여 반환할 수 있습니다. 적절한 클라이언트사이드 웹 프레임워크로 렌더링하기 위해 JSON, XML 등의 형식으로 데이터를 반환할 수도 있습니다 (이로 인해 서버의 처리 부담과 전송해야 하는 데이터의 양이 줄어 듭니다).
